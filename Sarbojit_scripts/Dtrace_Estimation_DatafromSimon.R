...
for (itr in 1:ITER) {
  print(itr)
  X = read.table(
    '/Users/roys/Downloads/TSdata_d100_cpp.txt',
    sep = ' ',
    nrows = d * n1,
  ...
  est1.D = parLapply(cl, 1:length(Big.S1), function(val) {
    #estimates Dk for all k. returns a list of estimated matrices.
    nlambda = 20 # length of lambda sequence
  ...
  #------------------------------------------------------------------------
  normd[[itr]] = sapply(1:length(est1.D), function(val)
    norm(est1.D[[val]] - unfold.D[[val]], type = 'F')) #distance between the true matrix and the estimated matrix at every fourier frequencies
  estD.Fnorm[[itr]] = sapply(est1.D, norm, type = 'F') # Frobenius norm of the estimated Dk-s
  
}
plot(Reduce('+', normd) / ITER)
plot(Reduce('+', estD.Fnorm) / ITER)
plot(true.Fnorm)
library(plot.matrix)
par(mar=c(4,4,4,4))
plot(unfold.D[[1]]) #true pattern of sparsity
